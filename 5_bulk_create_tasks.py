...
# =====================================================
API_URL    = "https://<campaign>-hcm.digit.org/project/task/v1/bulk/_create"  # Update campaign domain
AUTH_TOKEN = ""                      # Paste auth token from OAuth login
TENANT_ID  = ""                      # e.g. "ko"
INPUT_JSON = "tasks_output_KOGI.json"  # Output from script 4_transform_es_to_payload.py
    ...
    for index, batch in enumerate(chunks(tasks, BATCH_SIZE), start=1):
        ...
        try:
            ...
            if response.status_code in (200, 201):
                print(f"✅ Batch {index} | Inserted: {len(batch)}")
                success += len(batch)
            else:
...
if __name__ == "__main__":
    ...
    print(f"📦 Total tasks to ingest: {len(task_data)}")
